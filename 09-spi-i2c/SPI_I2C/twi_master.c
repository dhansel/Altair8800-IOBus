/*
  ...
  Adapted for master-only operation and much reduced memory
  requirements by David Hansel 2022
...
static volatile uint8_t twi_timed_out_flag = 0;  // a timeout has been seen
static volatile uint8_t twi_do_reset_on_timeout = 0;  // reset the TWI registers on timeout

static uint8_t *twi_buffer;
static volatile uint8_t twi_bufferIndex;
static volatile uint8_t twi_bufferLength;
 ...
 *          sendStop: Uint8_Tean indicating whether to send a stop at the end
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  ...
  // set buffer in which to receive the data
  twi_buffer = data;
  ...
  twi_bufferIndex = 0;
  twi_bufferLength = length-1;  // This is not intuitive, read on...
  ...
  if (twi_bufferIndex < length) {
    length = twi_bufferIndex;
  }
 ...
 *          wait: uint8_tean indicating to wait for write or not
 *          sendStop: uint8_tean indicating whether or not to send a stop at the end
 * Output   0 .. success
...
uint8_t twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
  ...
  // set buffer from which to send the data
  twi_buffer = data;
 ...
 * Output   none
 */
void twi_setTimeoutInMicros(uint32_t timeout, uint8_t reset_with_timeout){
  twi_timed_out_flag = 0;
 ...
 * Output   none
 */
void twi_handleTimeout(uint8_t reset){
 ...
 * Desc     returns 1 if twi has seen a timeout
 *          optionally clears the timeout flag
 * Input    clear_flag: 1 if we should reset the hardware
 * Output   none
 */
uint8_t twi_manageTimeoutFlag(uint8_t clear_flag){
  uint8_t flag = twi_timed_out_flag;
  if (clear_flag){
    twi_timed_out_flag = 0;
  }
  return(flag);
